	...
	for k in possibleTags:
		if "path81_2" in tags[k]:
			...
			if msg not in badTags:
				log("?, " + k + "("+tags[k]["address"]+") = " + tags[k]["possible"])
				noBad += 1				
				badTags.append(msg)				
	...
	for k in unknownTags:
		if "path81_2" in tags[k]:
			...
			if address != "N/A":
				msg = k+"("+tags[k]["address"]+")"
				if msg not in badTags:
					log("Bad, " + k + "("+tags[k]["address"]+") = " + tags[k]["address"])
					noBad += 1				
					badTags.append(msg)								
...
def process79To81(filterTags79,filterTag81,sites):
	...
	query = "select tag_path_old,tag_path_new,address_new from vw_ign_tagxref where tag_path_old like ? and tag_path_old not like '%plunger%'"
	xrefPathTags={}
	...
	for i in range(dataset.getRowCount()):
		...
		if oldPath not in xrefPathTags:
			xrefPathTags[oldPath]=[]
		xrefPathTags[oldPath].append(str(newPath)+"("+str(address)+")")
...
#							print "Got Match " + k	
				...
				if match==False:
					...
					filterTags79[k]["possible"]=possibleTags
